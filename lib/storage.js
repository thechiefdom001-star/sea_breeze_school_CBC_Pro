export const Storage = {
    ...
    deduplicateData(data) {
        ...
        const uniqueStudents = students.filter(s => {
            ...
            if (!key) return false;
            if (studentSeen.has(key)) return false;
    ...
    mergeData(existingData, incomingData, type) {
        ...
        switch (type) {
            case 'students': {
                ...
                const existingIds = new Set(existing.map(s => String(s.admissionNo || s.id).toLowerCase().trim()));
                
                const incoming = (incomingData.students || []).filter(s => {
                    const key = String(s.admissionNo || s.id || '').toLowerCase().trim();
                    return key && !existingIds.has(key);
                });
                
                const merged = [...existing, ...incoming];
                console.log(`[Storage] Students merged: ${existing.length} local + ${incoming.length} new = ${merged.length}`);
    ...
    normalizeAssessments(assessments) {
        ...
        })).filter(a => !isNaN(a.score) && (a.studentId || a.studentAdmissionNo));
    },
