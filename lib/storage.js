export const Storage = {
    ...
    load() {
        try {
            ...
            return this.ensureDataIntegrity(parsed);
        } catch (e) {
    ...
    deduplicateData(data) {
        ...
        for (let i = assessments.length - 1; i >= 0; i--) {
            ...
            if (!assessKeys.has(key)) {
                assessKeys.add(key);
                uniqueAssessments.unshift(a);
            }
        }
        
        // Deduplicate students by admissionNo
    ...
    save(data) {
        try {
            ...
            const dataToSave = this.ensureDataIntegrity(data);
    ...
    getStudentFinancials(student, payments, settings) {
        ...
        // We filter by student ID. Payments can be tagged with the grade they were made in.
    ...
    replaceWithGoogleData(existingData, googleData) {
        ...
        // Deduplicate students by admissionNo (keep the last occurrence, which is generally the most recently updated)
        const uniqueStudents = [];
        const seenAdmNos = new Set();
        for (let i = normalizedStudents.length - 1; i >= 0; i--) {
            const s = normalizedStudents[i];
            if (!seenAdmNos.has(s.admissionNo)) {
                seenAdmNos.add(s.admissionNo);
                uniqueStudents.unshift(s); // unshift to keep original chronological order
            }
        }
        
        
        // Deduplicate assessments by unique key
        ...
        // Deduplicate and normalize payments
    ...
    mergeData(existingData, incomingData, type) {
        ...
        switch (type) {
            ...
            case 'all': {
                ...
                const uniqueIncomingAtt = incomingAttendance.filter((a, index, arr) =>
                    arr.findIndex(x => `${x.studentId}-${x.date}` === `${a.studentId}-${a.date}`) === index
                );
                const mergedAttendance = [...existingAttendance];
                uniqueIncomingAtt.forEach(a => {
                    const key = `${a.studentId}-${a.date}`;
                    const idx = mergedAttendance.findIndex(x => `${x.studentId}-${x.date}` === key);
                    if (idx > -1) {
                        mergedAttendance[idx] = { ...mergedAttendance[idx], ...a };
                    } else {
                        mergedAttendance.push(a);
                    }
                });
                // Deduplicate final merged attendance
                const dedupedAttendance = mergedAttendance.filter((a, index, arr) => 
                    arr.findLastIndex(x => `${x.studentId}-${x.date}` === `${a.studentId}-${a.date}`) === index
                );
    ...
    getSubjectsForGrade(grade, student = null) {
        ...
        if (grade === 'PP1' || grade === 'PP2') {
            return ['Mathematics activities', 'Language activities', 'Environmental Activities', 'Creative Activities', 'Religious Education Activities'];
