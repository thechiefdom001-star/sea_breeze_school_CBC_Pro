...
class GoogleSheetSync {
    ...
    async pushStudent(student) {
        ...
        try {
            ...
            const preparedStudent = this._prepareStudentForSheet(student);
            url.searchParams.set('data', JSON.stringify({ student: preparedStudent }));
    ...
    async deleteRecord(sheetName, recordId) {
        ...
        try {
            const url = new URL(this.settings.googleScriptUrl);
            url.searchParams.set('action', 'deleteRecord');
            url.searchParams.set('data', JSON.stringify({ sheetName, recordId }));

            const response = await fetch(url.toString());
            const text = await response.text();
            try {
                const result = JSON.parse(text);
                return result.success ? { success: true } : { success: false, error: result.error || 'Delete failed' };
            } catch {
