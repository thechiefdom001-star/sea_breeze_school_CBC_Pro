...
class GoogleSheetSync {
    ...
    async setActiveUser(deviceId) {
        if (!this.isConfigured() || !deviceId) return;

        try {
            const url = new URL(this.settings.googleScriptUrl);
            url.searchParams.set('action', 'setActive'); // MATCH GAS action
            url.searchParams.set('device', deviceId);   // MATCH GAS param
    ...
    async getActiveUsers() {
        if (!this.isConfigured()) return { success: true, count: 0, users: [] };
        ...
        try {
            ...
            try {
                const result = JSON.parse(text);
                // GAS returns activeCount and activeUsers
                return { 
                    success: true, 
    ...
    async registerTeacher(username, password, teacherId = '', name = '', role = 'teacher') {
        if (!this.isConfigured()) {
            return { success: false, error: 'Google Sheet not configured' };
        }
    ...
    async loginTeacher(username, password) {
        ...
        try {
            ...
            try {
                const result = JSON.parse(text);
                if (result.success) {
                    console.log('[Auth] Teacher logged in:', username);
                }
                return result;
            } catch {
    ...
    async getActivitySummary(days = 7) {
        ...
        try {
            ...
            try {
                const result = JSON.parse(text);
                return result || { total: 0, byAction: {}, byModule: {}, byUser: {} };
            } catch {
                return { total: 0, byAction: {}, byModule: {}, byUser: {} };
            }
        } catch (error) {
    ...
    async trackActivity(action, module, recordId, recordName, details, oldValue, newValue) {
        const userId = localStorage.getItem('et_teacher_session') 
            ? JSON.parse(localStorage.getItem('et_teacher_session')).username 
            : (localStorage.getItem('et_is_admin') === 'true' ? 'admin' : 'guest');
        const userName = localStorage.getItem('et_teacher_session')
            ? JSON.parse(localStorage.getItem('et_teacher_session')).name
            : 'Administrator';
        const userRole = localStorage.getItem('et_teacher_session') ? 'teacher' : 'admin';
