...
class GoogleSheetSync {
    ...
    async detectDeletions(sheetName, localRecords) {
        ...
        try {
            const url = new URL(this.settings.googleScriptUrl);
            url.searchParams.set('action', 'getDeletedIds');
            url.searchParams.set('sheetName', sheetName);
            url.searchParams.set('localIds', JSON.stringify((localRecords || []).map(r => String(r.id))));
            ...
            try {
                ...
                return {
                    ...
                    deletionCount: result.deletedIds?.length || 0
                };
            } catch {
                return { success: false, deletedIds: [], deletionCount: 0 };
            }
        } catch (error) {
            return { success: false, error: error.message, deletedIds: [], deletionCount: 0 };
        }
    }
